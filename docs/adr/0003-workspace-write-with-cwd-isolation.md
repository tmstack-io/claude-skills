...
Status: superseded by ADR 0004（cwd 分離は TUI 輸送路の trust ダイアログを回避できないと実測で確定した。ADR 0002 の Consequence「sandbox 既定は奏者単位（implement 奏者のみ workspace-write）」を置き換えた ADR であり、全奏者 workspace-write の部分は 0004 でも維持）
